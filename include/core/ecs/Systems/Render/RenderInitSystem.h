...
public:
  ...
  void Init() override {
    ...
    auto cameraComponent = std::make_shared<Components::CameraComponent>(
        Components::CameraComponent(projection));

    auto position = glm::vec3(0.0f, 0.0f, 2.0f);
    ...
    m_World->AddComponent<Components::CameraComponent>(entityCamera,
                                                       cameraComponent);

    auto renderComponent = std::make_shared<Components::RenderComponent>(
        Components::RenderComponent(m_Window, m_Context));
