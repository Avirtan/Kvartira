...
class World : public std::enable_shared_from_this<World> {
  ...
  public:
    World(std::uint32_t countEntity = 1000) : m_CountUpdatePool(1) {
    ...
    Ecs::Entity CreateEntity() {
        if (m_EntitiesPool.empty()) {
            ...
            for (std::size_t i = m_CountEntity; i < m_CountEntity * m_CountUpdatePool; i++) {
                m_EntitiesPool.push(i);
            }
            m_CountEntity = m_CountEntity * m_CountUpdatePool;
        }
    ...
    template <typename T> void AddComponent(Ecs::Entity &entity, std::shared_ptr<T> &component) {
    ...
    template <typename T> std::shared_ptr<T> GetComponent(Ecs::Entity &entity) {
        ...
        auto component = std::static_pointer_cast<T>(it->second->GetComponent<T>(entity));
        return component;
    }
    ...
    template <typename T, typename K> std::vector<Ecs::Entity> GetPoolComponent() {
  ...
  private:
    ...
    std::unordered_map<std::string, std::shared_ptr<Ecs::ArrayComponent>> m_Components;
